...
<p><strong>Returned Payment Fee.</strong> If any check or electronic payment tendered by Tenant is returned for insufficient funds or for any other reason, Tenant shall pay to Landlord, in addition to the Late Fee and Default Interest described above, a returned payment fee of <strong>{{lease.returnedPaymentFee}}</strong>. {{#if lease.returnedCheckPaymentRestriction}}If two (2) or more checks or electronic payments are returned for any reason within any rolling twelve (12) month period, Tenant shall thereafter be required to make all Rent payments exclusively by money order, cashier&rsquo;s check, or cash for the remainder of the Lease Term. This requirement shall take effect upon Landlord&rsquo;s written notice to Tenant following the second returned payment and shall remain in effect for the duration of the Lease.{{/if}}{{#unless lease.returnedCheckPaymentRestriction}}After two (2) or more returned payments during any twelve (12) month period, Landlord may require Tenant to make all future Rent payments by cashier&rsquo;s check, certified check, or wire transfer.{{/unless}}</p>
