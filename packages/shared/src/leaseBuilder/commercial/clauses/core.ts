...
<p><strong>Approximate Rentable Square Footage:</strong> {{premises.sqft}} square feet{{#if premises.sqftApproximate}}<sup>*</sup>{{/if}}</p>
{{#if premises.sqftApproximate}}<p><small><sup>*</sup> The square footage stated herein is approximate and based on available records. The parties acknowledge that actual dimensions may vary, and this Lease shall not be subject to adjustment on account of any such difference.</small></p>{{/if}}
...
<p>The term of this Lease (the &ldquo;Term&rdquo;) shall commence {{#if lease.startDateApproximate}}on or about{{/if}}{{#unless lease.startDateApproximate}}on{{/unless}} <strong>{{lease.startDate}}</strong>{{#if lease.startDateApproximate}} (approximate){{/if}} (the &ldquo;Commencement Date&rdquo;) and shall expire on <strong>{{lease.endDate}}</strong> (the &ldquo;Expiration Date&rdquo;), unless sooner terminated in accordance with the provisions of this Lease.</p>
{{#if lease.startDateCondition}}<p>Notwithstanding the foregoing, the Commencement Date shall not occur until the following condition has been satisfied: <strong>{{lease.startDateCondition}}</strong>. Upon satisfaction of such condition, the parties shall promptly execute a Commencement Date Memorandum confirming the actual Commencement Date and Expiration Date.</p>{{/if}}
{{#unless lease.startDateCondition}}<p>The &ldquo;Rent Commencement Date&rdquo; shall be {{lease.rentCommencementDate}}. If the Rent Commencement Date differs from the Commencement Date, Tenant shall have access to the Premises from the Commencement Date for the purpose of performing any approved tenant improvements, installing fixtures, and preparing the Premises for occupancy, subject to all terms and conditions of this Lease except the obligation to pay Base Rent.</p>{{/unless}}
{{#if lease.startDateCondition}}<p>The &ldquo;Rent Commencement Date&rdquo; shall be the date on which Landlord delivers possession of the Premises to Tenant following satisfaction of the condition described above, which the parties anticipate to occur on or about <strong>{{lease.rentCommencementDate}}</strong>. No Base Rent shall be due or payable prior to the satisfaction of such condition and Landlord&rsquo;s delivery of possession to Tenant. If the Rent Commencement Date differs from the Commencement Date, Tenant shall have access to the Premises from the Commencement Date for the purpose of performing any approved tenant improvements, installing fixtures, and preparing the Premises for occupancy, subject to all terms and conditions of this Lease except the obligation to pay Base Rent.</p>{{/if}}
...
<ol>
  ...
  <li>{{#if lease.renewalRentAtMarket}}Base Rent during each Renewal Term shall be reset to the then-prevailing fair market rental rate for comparable commercial space in the same market area, as mutually agreed upon by the parties in writing within thirty (30) days of Tenant&rsquo;s exercise of the Renewal Option or, failing such agreement, as determined by an independent MAI-certified appraiser selected by mutual agreement of the parties;{{/if}}{{#unless lease.renewalRentAtMarket}}Base Rent during each Renewal Term shall be adjusted in accordance with the rent escalation provisions of this Lease, continuing from the Base Rent payable in the last year of the immediately preceding term;{{/unless}} and</li>
