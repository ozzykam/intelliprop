...
export default function EditOrganizationPage({ params }: { params: Promise<{ orgId: string }> }) {
    ...
    fetch(`/api/admin/organizations/${orgId}`)
      ...
      .then(data => {
        if (data.ok) {
          ...
          setError(data.error?.message ?? 'Failed to load organization');
        }
      })
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        <Link href={`/main/organizations/${orgId}`} className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
      ...
      <form onSubmit={handleSubmit} className="space-y-5 border rounded-xl p-6 bg-card">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <Link href={`/main/organizations/${orgId}`} className="px-4 py-2 text-sm border rounded-md hover:bg-muted transition-colors">
            Cancel
          </Link>
          <button
            type="submit"
