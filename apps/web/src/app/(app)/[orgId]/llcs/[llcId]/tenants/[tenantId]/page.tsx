...
interface EditTenantPageProps {
  params: Promise<{ orgId: string; llcId: string; tenantId: string }>;
}
...
export default function EditTenantPage({ params }: EditTenantPageProps) {
  const { orgId, llcId, tenantId } = use(params);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.ok) {
        router.push(`/${orgId}/llcs/${llcId}/tenants`);
      } else {
