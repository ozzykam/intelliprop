...
interface StandaloneAppraisalPageProps {
...
export default function StandaloneAppraisalPage({ params }: StandaloneAppraisalPageProps) {
  const { orgId, llcId, claimId } = use(params);
  const basePath = `/${orgId}/llcs/${llcId}/insurance/claims/${claimId}`;
  ...
  const [awardSignedBy, setAwardSignedBy] = useState<NonNullable<AppraisalProcess['award']>['signedBy']>('both_appraisers');
      ...
      <div className="border rounded-lg p-5 space-y-4">
        ...
        {!showPanelEdit ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <PanelMemberCard title="Insured's Appraiser" subtitle="Chosen by policyholder" member={panel.insuredAppraiser} />
            <PanelMemberCard title="Insurer's Appraiser" subtitle="Chosen by carrier" member={panel.insurerAppraiser} />
            <PanelMemberCard
              title="Umpire"
    ...
    <div className={`border rounded-lg p-4 ${member ? 'bg-card' : 'bg-secondary/20 border-dashed'}`}>
      ...
      {member ? (
        <>
          ...
          {member.licenseNumber && <p className="text-xs text-muted-foreground">Lic #{member.licenseNumber}</p>}
...
function AppraiserForm({
  title, name, onName, firm, onFirm, phone, onPhone, email, onEmail, license, onLicense, date, onDate,
}: {
  title: string;
