...
interface StandaloneClaimPageProps {
  params: Promise<{ orgId: string; llcId: string; claimId: string }>;
}
...
export default function StandaloneClaimDetailPage({ params }: StandaloneClaimPageProps) {
  const { orgId, llcId, claimId } = use(params);
  const router = useRouter();
  const insurancePath = `/${orgId}/llcs/${llcId}/insurance`;
  const selfPath = `${insurancePath}/claims/${claimId}`;
  ...
  async function handleInitiateAppraisal(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.ok) {
        ...
        router.push(`${selfPath}/appraisal`);
      } else {
  ...
  if (error || !claim) {
    return (
      <div className="p-6">
        <Link href={insurancePath} className="text-sm text-muted-foreground hover:text-foreground">← Insurance</Link>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <Link href={insurancePath} className="text-sm text-muted-foreground hover:text-foreground">
          ← Insurance
        </Link>
        <div className="flex items-start justify-between mt-2 gap-4">
          <div>
            ...
            {claim.policyId && (
              <Link
                href={`${insurancePath}/${claim.policyId}`}
      ...
      {tab === 'overview' && (
        <div className="space-y-4">
          <div className="border rounded-lg p-5 grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
            {claim.carrier && <InfoField label="Carrier" value={claim.carrier} />}
            {claim.policyNumber && <InfoField label="Policy Number" value={claim.policyNumber} />}
      ...
      {tab === 'parties' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <Link href={`${selfPath}/edit`} className="text-sm text-primary hover:underline">
              Edit Parties
            </Link>
          </div>
          ...
          {(claim.adjusters?.length ?? 0) === 0 &&
            ...
            <div className="border rounded-lg p-8 text-center">
              ...
              <Link href={`${selfPath}/edit`} className="text-sm text-primary hover:underline mt-1 inline-block">
          ...
          {showUploadForm && (
            <form onSubmit={handleUpload} className="border rounded-lg p-4 space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">PDF, JPG, PNG, WebP, HEIC, MP4, MOV, DOC, DOCX</p>
              </div>
            ...
            <div className="space-y-2">
              {documents.map(doc => (
                <div key={doc.id} className="border rounded-lg p-4 flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => handleDownload(doc)} className="text-sm text-primary hover:underline">
                      Download
                    </button>
                    <button onClick={() => handleDeleteDoc(doc.id)} className="text-sm text-destructive hover:underline">
                      Delete
                    </button>
                  </div>
                </div>
