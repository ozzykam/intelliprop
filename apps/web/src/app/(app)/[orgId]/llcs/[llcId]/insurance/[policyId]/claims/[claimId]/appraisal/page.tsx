...
interface AppraisalPageProps {
  params: Promise<{ orgId: string; llcId: string; policyId: string; claimId: string }>;
}
...
export default function AppraisalPage({ params }: AppraisalPageProps) {
  const { orgId, llcId, policyId, claimId } = use(params);
  const basePath = `/${orgId}/llcs/${llcId}/insurance/${policyId}/claims/${claimId}`;
