...
interface EditMemberPageProps {
  params: Promise<{ orgId: string; llcId: string; userId: string }>;
}
...
export default function EditMemberPage({ params }: EditMemberPageProps) {
  const { orgId, llcId, userId } = use(params);
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      if (data.ok) {
        router.push(`/${orgId}/llcs/${llcId}/members`);
      } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-between pt-4">
          <div className="flex gap-3">
            ...
            <button
              type="button"
              onClick={() => router.push(`/${orgId}/llcs/${llcId}/members`)}
